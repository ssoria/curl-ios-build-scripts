  ...
  class Compiler < ConfigurableStep
    ...
    def compilation_flags_for(platform, architecture)
      if platform == "iPhoneSimulator"
        ...
        min_version = "-mmacosx-version-min=10.9"
      end
...

      {
        ...
        cflags:  "-arch #{architecture} -pipe -Os -gdwarf-2 -isysroot #{sdk} #{min_version} #{embed_bitcode} -Werror=partial-availability"
      }
    end
    ...
    def configure(architecture, tools, compilation_flags)
      ...
      ios_path = `xcrun -sdk iphoneos --show-sdk-path`.strip
      ios_simulator_path = `xcrun -sdk iphonesimulator --show-sdk-path`.strip
      ios_sdk_path = (architecture != "x86_64" ? ios_path : ios_simulator_path)
